...
\paragraph{Required attributes.} \texttt{keyid} (identifies the signer, resolvable per \S2.2), \texttt{signature} (Base64-encoded cryptographic signature), and \texttt{content-hash} (hash of the canonicalized content, prefixed with the algorithm, e.g. \texttt{sha256:\ldots}).

\paragraph{Optional attributes.} \texttt{algorithm} (defaults to \texttt{ed25519} when omitted). Visual presentation (\texttt{style}, \texttt{class}) SHOULD be handled by user-agent CSS rather than inline attributes to keep the tag compact.
...
\paragraph{Canonical content extraction.} The hash over which the signature is computed is taken from the \textit{text content} of the signed region after the extraction and normalization process defined in \S2.3. Markup and attributes of the signed content itself are not covered by the hash; readers consume text, and HTMLTrust attests to the integrity of that text. A future revision MAY define optional extensions for covering specific attributes (e.g., link destinations) where the semantic integrity of non-text elements is relevant.
